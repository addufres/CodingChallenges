...
(e.g. if we are working with page 102 but have ink only for two digits then this page will 
...
public class PageNumbering {
	...
	public PageNumbering(int current, int numberOfDigits) {
		super();
	...
	public void pagesNumberingWithInk(int current, int numberOfDigits) {
	    int currentDigits = countDigitsInNumber(current); 
	    while (numberOfDigits >= currentDigits) {
	        numberOfDigits -= currentDigits;
	        current++;
	        currentDigits = countDigitsInNumber(current);
	    }
	    System.out.println(current - 1);
	}
	private int countDigitsInNumber(int n) {
	    int count = 0;
	    while (n > 0) {
	        count++;
	        n /= 10;
